...
describe('nhlFetch', () => {
  ...
  test('prepends slash if missing and returns JSON', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/scoreboard/now'), expect.any(Object));
  });

  test('auto adds leading slash', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ hi: 1 }) });
    await nhlFetch('scoreboard/now');
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/scoreboard/now'), expect.any(Object));
  });

  test('throws detailed error on !ok', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({ ok: false, status: 500, statusText: 'Server', text: () => Promise.resolve('Bad things'), json: () => Promise.resolve({}) });
...
describe('nhlStatic', () => {
  ...
  (global.fetch as any) = jest.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ via: 'static' }) });
