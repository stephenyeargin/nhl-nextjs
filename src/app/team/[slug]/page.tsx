...
interface TeamStanding {
  conferenceSequence?: number; conferenceName?: string;
  divisionSequence?: number; divisionName?: string;
  gamesPlayed?: number; wins?: number; losses?: number; otLosses?: number;
  points?: number; pointPctg?: number; regulationWins?: number; regulationPlusOtWins?: number;
  goalFor?: number; goalAgainst?: number; goalDifferential?: number;
  homeWins?: number; homeLosses?: number; homeOtLosses?: number;
  roadWins?: number; roadLosses?: number; roadOtLosses?: number;
  shootoutWins?: number; shootoutLosses?: number;
  l10Wins?: number; l10Losses?: number; l10OtLosses?: number;
...
export default async function SchedulePage(props: any) {
  ...
  const [teamStatsResponse, standingsResponse, scheduleResponse, fullSeasonScheduleResponse, newsResponse] = await Promise.all([
    ...
    fetch(`https://api-web.nhle.com/v1/club-schedule-season/${team.abbreviation}/now`, { cache: 'no-store' }),
    fetch(`https://forge-dapi.d3.nhle.com/v2/content/en-us/stories?tags.slug=teamid-${team.teamId}&context.slug=teamid-${team.teamId}&$limit=8`, { cache: 'no-store' }),
  ]);
  ...
  const teamStanding: TeamStanding | undefined = standings.standings.find((standing: TeamStanding) => {
    const abbrev = typeof standing.teamAbbrev === 'string' ? standing.teamAbbrev : standing.teamAbbrev?.default;
  ...
  return (
    <div className="container mx-auto px-2 py-8">
        ...
        <div className="mb-5 overflow-x-scroll scrollbar-hidden">
          <div className="flex flex-row mb-5 gap-2">
            {([
              ...
              { label: 'Overall Record', value: `${teamStanding.wins}-${teamStanding.losses}-${teamStanding.otLosses}` },
              ...
              { label: 'Home Record', value: `${teamStanding.homeWins}-${teamStanding.homeLosses}-${teamStanding.homeOtLosses}` },
              { label: 'Road Record', value: `${teamStanding.roadWins}-${teamStanding.roadLosses}-${teamStanding.roadOtLosses}` },
              { label: 'Shootout Record', value: `${teamStanding.shootoutWins}-${teamStanding.shootoutLosses}` },
              { label: 'Last 10 Record', value: `${teamStanding.l10Wins}-${teamStanding.l10Losses}-${teamStanding.l10OtLosses}` },
              { label: 'Streak', value: `${teamStanding.streakCode}${teamStanding.streakCount}` },
            ] as const).map((stat, i) => (
              <div key={i} className="flex flex-col p-2 bg-transparent text-center border rounded content-center" style={{minWidth: '7rem'}}>
      ...
      <div className="mb-5">
        ...
        <StatsTable stats={teamStats.skaters.filter((t: any) => t.positionCode !== 'D')} team={team.abbreviation} />
