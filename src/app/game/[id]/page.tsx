...
const GamePage: React.FC = () => {
  ...
  if (pageError) {
    ...
    const safePageError = { message: (pageError as any).message || (pageError as any).error?.message };
              ...
              <div key={index} className="mb-2">
                ...
                {period.periodDescriptor.periodType === 'SO' ? (
                  <>
                    ...
                    {game.summary?.shootout?.length === 0 && <p className="text-slate-500">No shots taken.</p>}
                          ...
                          <div className="grow">
                            ...
                            <div className="col-span-10 flex">
                              ...
                              <span className="capitalize">
                                {shot.shotType} • {(SHOOTOUT_RESULT as Record<string, string>)[shot.result] || shot.result}
                              </span>
                            </div>
                          </div>
                  ...
                  <>
                    ...
                    {period.goals.length > 0 && period.goals.map((goal: any, i: number) => (
                          ...
                          <div>
                            ...
                            {goal.goalModifier !== 'none' && (
                              <span
                                ...
                                title={(GOAL_MODIFIERS as Record<string, { title: string; label: string }>)[goal.goalModifier]?.title}
                              >
                                {(GOAL_MODIFIERS as Record<string, { title: string; label: string }>)[goal.goalModifier]?.label}
                              </span>
                            )}
                            <br />
                              ...
                              <div className="text-sm"> {goal.assists.length > 0 ? (
                                <>
                                  ...
                                  {goal.assists.map((assist: any, ai: number) => (
                                    <span key={ai}>{assist.firstName?.default} {assist.lastName?.default} ({assist.assistsToDate}){ai !== goal.assists.length - 1 && ', '}</span>
                                  ))}
                                </>
                              ) : (
                                <>Unassisted</>
            ...
            <div className="my-10">
              ...
              {game.summary.penalties.map((period: any, index: number) => (
                <div key={index} className="mb-5">
                  <h4 className="font-semibold">{formatPeriodLabel({ ...game.periodDescriptor, number: index + 1 }, true)}</h4>
                    ...
                    <div className="min-w-full">
                      <div className="flex flex-col">
                        {period.penalties.map((penalty: any, penaltyIndex: number) => (
                          <div key={penaltyIndex} className={`my-1 flex ${penaltyIndex % 2 ? '' : 'bg-slate-500/10'}`}>
                            <div className="w-20 p-4 text-right">
                              <span className="m-1 border rounded p-1 font-bold text-xs">{penalty.timeInPeriod}</span>
                            </div>
                            <div className="w-1/3 p-2">
                              <div className="flex flex-wrap">
                                <TeamLogo src={logos[penalty.teamAbbrev.default]} alt="Logo" className="w-10 h-10 mr-2" team={penalty.teamAbbrev.default} />
                                <div>
                                  <div className="font-bold">{penalty.committedByPlayer?.default || penalty.teamAbbrev.default}</div>
                                  {penalty.drawnBy && <div className="text-xs text-slate-600">Drawn by: {penalty.drawnBy.default}</div>}
                                  {penalty.servedBy && <div className="text-xs text-slate-600">Served by: {penalty.servedBy.default}</div>}
                                </div>
                              </div>
                            </div>
                            <div className="w-30 p-4">{penalty.duration ? `${penalty.duration} mins` : '-'}</div>
                            <div className="w-1/4 p-2">
                              <div className="text-xs font-light text-slate-600">{(PENALTY_TYPES as Record<string, string>)[penalty.type] || penalty.type}</div>
                              <div className="text-sm sm:text-base">{(PENALTY_DESCRIPTIONS as Record<string, string>)[penalty.descKey] || penalty.descKey.replace(/-/g, ' ')}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {summary.threeStars.map((p: any) => (
                  <div key={p.playerId} className="text-center py-4">
                    <div className="relative inline-block">
                      <span className="absolute bottom-0 left-0 bg-white text-black rounded-full font-bold border border-slate-200 w-8 h-8 flex items-center justify-center">{p.star}</span>
                      <Headshot playerId={p.playerId} src={p.headshot} alt={p.name.default} size="6" className="mx-auto mb-2" team={p.teamAbbrev} />
                    </div>
                    <h4 className="font-semibold"><Link href={`/player/${p.playerId}`}>{p.name.default}</Link></h4>
