...
const PlayByPlay: React.FC = () => {
  ...
  const renderTeamLogo = (teamId?: number | string, size?: number, theme?: string) => {
    ...
    if (teamId === awayTeam.id) {
      return <TeamLogo src={logos[awayTeam.abbrev]} alt={awayTeam.abbrev} className={className} colorMode={theme as any} />;
    }
    if (teamId === homeTeam.id) {
      return <TeamLogo src={logos[homeTeam.abbrev]} alt={homeTeam.abbrev} className={className} colorMode={theme as any} />;
    }
  ...
  const renderPlayer = (playerId: number) => {
    ...
    if (!player.playerId) {
      return <><span className="font-bold">{player.firstName?.default} {player.lastName?.default}</span></>;
    }
    
  return <><span className="p-1 border rounded text-xs mx-1">#{player.sweaterNumber}</span> <Link href={`/player/${player.playerId}`} className="font-bold">{player.firstName?.default} {player.lastName?.default}</Link></>;
  };
  const renderPlayByPlayEvent = (play: any) => {
    ...
    switch (play.typeDescKey) {
      case 'period-start':
        return <div>Start of {formatPeriodLabel({ ...game.periodDescriptor, number: play.periodDescriptor.number }, true)}</div>;
      case 'faceoff':
  return <div>{renderPlayer(e.winningPlayerId)} won a faceoff against {renderPlayer(e.losingPlayerId)} {e.zoneCode ? `in the ${(ZONE_DESCRIPTIONS as Record<string,string>)[e.zoneCode]}` : ''}</div>;
      case 'blocked-shot':
  return <div>{renderPlayer(e.shootingPlayerId)}&apos;s shot was blocked by {e.reason === 'teammate-blocked' ? 'teammate' : ''} {renderPlayer(e.blockingPlayerId)}</div>;
      case 'shot-on-goal':
        return <div>{renderPlayer(e.shootingPlayerId)} took a shot on {renderPlayer(e.goalieInNetId)}</div>;
      case 'stoppage':
  return <div>{(GAME_EVENTS as Record<string,string>)[e.reason]}{(e.secondaryReason && e.secondaryReason !== e.reason) ? `, ${(GAME_EVENTS as Record<string,string>)[e.secondaryReason]}` : ''}</div>;
      case 'hit':
  return <div>{renderPlayer(e.hittingPlayerId)} hit {renderPlayer(e.hitteePlayerId)} {e.zoneCode ? `in the ${(ZONE_DESCRIPTIONS as Record<string,string>)[e.zoneCode]}` : ''}</div>;
      case 'giveaway':
  return <div>{renderPlayer(e.playerId)} gave the puck away {e.zoneCode ? `in the ${(ZONE_DESCRIPTIONS as Record<string,string>)[e.zoneCode]}` : ''}</div>;
      case 'takeaway':
  return <div>{renderPlayer(e.playerId)} took the puck away {e.zoneCode ? `in the ${(ZONE_DESCRIPTIONS as Record<string,string>)[e.zoneCode]}` : ''}</div>;
      case 'missed-shot':
  return <div>{renderPlayer(e.shootingPlayerId)} missed a {e.shotType} shot {e.reason ? `(${(MISS_TYPES as Record<string,string>)[e.reason] || e.reason})` : ''}</div>;
      case 'goal':
        return (
          <div className="p-2 gap-5 rounded-lg flex flex-wrap justify-center md:justify-between items-center bg-red-900/80 text-white" style={{ borderLeft: `solid 25px ${eventTeamData.teamColor}` }}>
            <div className="flex gap-5 items-center justify-start">
              ...
              />
              <div>
                <div className="font-medium text-lg mb-3">{renderPlayer(e.scoringPlayerId)} ({e.scoringPlayerTotal}) scored {e.shotType ? `(${(GAME_EVENTS as Record<string,string>)[e.shotType]?.toLowerCase()})` : ''}</div>
                {(e.assist1PlayerId || e.assist2PlayerId) && <span>Assisted By: </span>}
                {e.assist1PlayerId && <span>{renderPlayer(e.assist1PlayerId)} ({e.assist1PlayerTotal})</span>}
                {e.assist2PlayerId && <span> and {renderPlayer(e.assist2PlayerId)} ({e.assist2PlayerTotal})</span>}
        ...
        return (
          <div className="p-2 gap-5 rounded-lg flex flex-wrap justify-center md:justify-start items-center bg-slate-500/80 text-white" style={{ borderLeft: `solid 25px ${eventTeamData.teamColor}` }}>
            ...
            <div>
              ...
              {e.servedByPlayerId && <div className="text-xs my-2">Served by: {renderPlayer(e.servedByPlayerId)}</div>}
              {e.drawnByPlayerId && <div className="text-xs my-2">Drawn By: {renderPlayer(e.drawnByPlayerId)}</div>}
            </div>
            <div>
            ...
            <div>
              <div className="text-xs font-light">{(PENALTY_TYPES as Record<string,string>)[e.typeCode]}</div>
              <div>{(PENALTY_DESCRIPTIONS as Record<string,string>)[e.descKey]}</div>
            </div>
          </div>
        );
      case 'period-end':
        return <div>End of {formatPeriodLabel({ ...game.periodDescriptor, number: play.periodDescriptor.number }, true)}</div>;
  ...
  <PeriodSelector periodData={game.periodDescriptor} activePeriod={activePeriod || 0} handlePeriodChange={setActivePeriod} includeAll={true} />
        <div className="mx-2">
          <select className="p-2 min-w-[100px] md:min-w-[150px] border rounded text-black dark:text-white bg-inherit" value={eventFilter || 'all'} onChange={(e) => setEventFilter(e.target.value)}>
        ...
        <div>
          <select className="p-2 min-w-[100px] md:min-w-[150px] border rounded text-black dark:text-white bg-inherit" value={teamFilter || 'all'} onChange={(e) => setTeamFilter(e.target.value)}>
      ...
      <div className="overflow-x-auto">
        <table className="text-xs min-w-full table-auto">
          ...
          <tbody>
            {sortedPlays.length === 0 && <tr><td colSpan={3} className="p-2 text-center">No matching plays.</td></tr>}
            {sortedPlays.map((play: any, i: number) => (
              <tr key={play.eventId} className={i % 2 === 0 ? 'bg-slate-500/10' : ''}>
                <td className="p-2 text-xs text-center">
                  <div className="mt-1"><span className="p-1 mx-auto font-bold border rounded">{play.timeRemaining}</span></div>
                  {activePeriod === 0 && <div className="p-2">{formatPeriodLabel(play.periodDescriptor, true)}</div>}
                </td>
                <td className="p-2 flex flex-wrap gap-2 items-center">
                  <div className="w-10 h-10">{play.details?.eventOwnerTeamId && renderTeamLogo(play.details?.eventOwnerTeamId)}</div>
                  <div>
                    <span className="hidden lg:block p-1 border rounded font-bold text-xs uppercase">{(GAME_EVENTS as Record<string,string>)[play.typeDescKey]}</span>
                    {play.typeDescKey === 'goal' && (
                      <div>
                        <Image src={SirenOnSVG} className="p-2 w-12 h-12 animate-pulse mx-auto" width={200} height={200} alt="Goal" />
                      </div>
                    )}
                  </div>
                </td>
