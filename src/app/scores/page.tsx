          ...
          {scores.gameWeek.map((day, i) => (
            ...
            >
              <div className="text-xs my-1">
                {day.dayAbbrev} {formatLocalizedDate(day.date, 'M/D')}
              </div>
              <div className="text-xs my-1 font-bold">
      ...
      <div className="my-5">
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-1 text-start">
            <button
              onClick={() => handleDateChange(scores.prevDate)}
              className="font-bold underline"
            >
              &laquo; {formatLocalizedDate(scores.prevDate, 'LL')}
            </button>
          </div>
          <div className="col-span-1 text-center">
            {formatLocalizedDate(scores.currentDate, 'dddd, MMMM D, YYYY')}
          </div>
          {scores.nextDate ? (
            <div className="col-span-1 text-end">
              <button
                onClick={() => handleDateChange(scores.nextDate!)}
                className="font-bold underline"
              >
                {formatLocalizedDate(scores.nextDate, 'LL')} &raquo;
              </button>
            </div>
          ) : (
            <div className="col-span-1 text-end" />
