...
interface StoryContextValue {
  ...
  players: Tag[];
  sidebarStories: unknown; // keep loose; components treat as opaque list
...
const fetchStory = async (storyId: string): Promise<string | StoryEntity> => {
  ...
  if (isLegacyStory) {
    const homebaseContent: unknown = await fetchJsonData(
      `https://forge-dapi.d3.nhle.com/v2/content/en-us/stories/?fields.homebaseId=${storyId.replace('c-', '')}`
    );
    const hb = homebaseContent as { items?: Array<{ slug?: string }> };

    return hb.items?.[0]?.slug || storyId;
  }

  return fetchJsonData<StoryEntity>(
    `https://forge-dapi.d3.nhle.com/v2/content/en-us/stories/${storyId}`
  );
};

const fetchSidebarStories = () =>
  fetchJsonData(
...
export const StoryProvider: React.FC<StoryProviderProps> = ({ storyId, children }) => {
  ...
  const [sidebarStories, setSidebarStories] = useState<unknown>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const playerTags =
          storyResponse.tags?.filter((t) => t.externalSourceName === 'player') || [];
        ...
        formatHeadTitle(
          `${formatLocalizedDate(storyResponse.contentDate)}: ${storyResponse.headline || storyResponse.title}`
        );
      } catch (error) {
