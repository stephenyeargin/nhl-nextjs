...
interface TBDLogoProps {
  className?: string;
}
const TBDLogo: React.FC<TBDLogoProps> = ({ className }) => (
  <Image
    src="https://assets.nhle.com/logos/nhl/svg/team-tbd-light.svg"
    width="1024"
    height="1024"
    alt="To Be Determined"
    className={className || 'w-8 h-8 mr-3'}
  />
);
      ...
      <>
        ...
        >
          <div className="flex flex-wrap gap-5 justify-center">
            <div
              className={`text-center ${top && series.losingTeamId === top.id ? 'opacity-50' : ''}`}
            >
              {top && top.id !== -1 ? (
                <>
                  ...
                  <div>
                    ...
                    <span className="font-bold">
                      {top.commonName?.default && top.placeNameWithPreposition?.default
                        ? top.commonName.default.replace(
                            top.placeNameWithPreposition.default,
                            'TBD'
                          )
                        : top.commonName?.default || 'TBD'}
                    </span>
                  </div>
            ...
            <div
              className={`text-center ${bottom && series.losingTeamId === bottom.id ? 'opacity-50' : ''}`}
            >
              {bottom && bottom.id !== -1 ? (
                <>
                  <TeamLogo
                  ...
                  <div>
                    ...
                    <span className="font-bold">
                      {bottom.commonName?.default && bottom.placeNameWithPreposition?.default
                        ? bottom.commonName.default.replace(
                            bottom.placeNameWithPreposition.default,
                            'TBD'
                          )
                        : bottom.commonName?.default || 'TBD'}
                    </span>
                  </div>
    ...
    <>
      ...
      >
        <div className="flex-1">
          <div
            className={`flex items-center justify-center xl:justify-start ${top && series.losingTeamId === top?.id ? 'opacity-50' : ''}`}
          >
            {top && top.id !== -1 ? (
              <>
                ...
                <div className="hidden xl:block grow">
                  {top.commonName?.default && top.placeNameWithPreposition?.default
                    ? top.commonName.default.replace(top.placeNameWithPreposition.default, 'TBD')
                    : top.commonName?.default || 'TBD'}
                </div>
          ...
          <div
            className={`flex items-center justify-center xl:justify-start ${bottom && series.losingTeamId === bottom?.id ? 'opacity-50' : ''}`}
          >
            {bottom && bottom.id !== -1 ? (
              <>
