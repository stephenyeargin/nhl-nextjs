...
interface GameStoryProps {
  game: { id: number | string; gameState: string };
}
...
const GameStory: React.FC<GameStoryProps> = ({ game }) => {
  ...
  const type =
    game.gameState === 'FINAL' || game.gameState === 'OFF' ? 'game-recap' : 'game-preview';

  useEffect(() => {
    const fetchGameStory = async () => {
      const contentResponse = await fetch(
        `https://forge-dapi.d3.nhle.com/v2/content/en-us/stories?tags.slug=gameid-${game.id}&tags.slug=${type}&context.slug=nhl&$limit=1`
      );
