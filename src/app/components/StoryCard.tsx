      ...
      <div className={className}>
        <Link href={`/news/${item.slug}`} className="grid grid-cols-3 gap-4 items-center">
          ...
          <div className="col-span-2">
            ...
            {showDate && <div>{formatLocalizedDate(item.contentDate)}</div>}
          </div>
        </Link>
      </div>
    );
  ...
  if (size === 'medium') {
    return (
      <div className={className}>
        ...
        />
        <Link href={`/news/${item.slug}`} className="block font-bold py-3 underline">
          Read Story
        </Link>
      </div>
    );
    ...
    return (
      <div className={`md:relative ${className}`}>
        <Link href={`/news/${item.slug}`} className="">
          <Image
            src={
              (item.thumbnail?.templateUrl || '').replace(
                '{formatInstructions}',
    ...
    <div className={className}>
      ...
      {showDate && (
        <div className="text-xs text-gray-500 my-2">
          {formatLocalizedDate(item.contentDate, 'LL')}
        </div>
      )}
      <div
        className="text-justify line-clamp-3 text-sm"
