...
const row = (overrides: Partial<any>): any => ({
  ...
  goalAgainst: 25,
  goalDifferential: 5,
  homeWins: 3, homeLosses: 1, homeOtLosses: 1,
  roadWins: 2, roadLosses: 2, roadOtLosses: 1,
...
describe('StandingsTable', () => {
  it('sorts and swaps first six rows when swap condition met and renders wildcard rankings + clinch indicator opacity', () => {
    ...
    const rows = [
      row({ teamAbbrev: { default: 'R1' }, teamName: { default: 'Team R1' }, points: 10, wildcardSequence: 1 }),
      row({ teamAbbrev: { default: 'R2' }, teamName: { default: 'Team R2' }, points: 11, wildcardSequence: 1 }),
      row({ teamAbbrev: { default: 'R3' }, teamName: { default: 'Team R3' }, points: 12, wildcardSequence: 1 }),
      row({ teamAbbrev: { default: 'R4' }, teamName: { default: 'Team R4' }, points: 40, wildcardSequence: 2, clinchIndicator: 'e' }), // higher points -> will move up after swap
      row({ teamAbbrev: { default: 'R5' }, teamName: { default: 'Team R5' }, points: 39, wildcardSequence: 2 }),
      row({ teamAbbrev: { default: 'R6' }, teamName: { default: 'Team R6' }, points: 38, wildcardSequence: 2 }),
    ];
