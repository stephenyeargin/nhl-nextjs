...
jest.mock('./TeamLogo', () => {
  type TeamLogoMockProps = { colorMode?: string } & React.HTMLAttributes<HTMLDivElement>;
  const Mock: React.FC<TeamLogoMockProps> = ({ colorMode: _colorMode, ...rest }) => (
    <div data-testid="team-logo" {...rest}>
      Logo
    </div>
  );
...
describe('LeagueToggle', () => {
  ...
  it('switches active league color path', () => {
    ...
    const { rerender } = render(
      <LeagueToggle activeLeague="other" handleChangeLeagues={handler} activeColor="#ffffff" />
    );
    ...
    rerender(
      <LeagueToggle activeLeague="nhl" handleChangeLeagues={handler} activeColor="#ffffff" />
    );
    const nhlBtn = screen.getByRole('button', { name: /National Hockey League/i });
