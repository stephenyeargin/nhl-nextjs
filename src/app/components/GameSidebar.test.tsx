...
jest.mock('./Scoreboard', () => ({ __esModule: true, default: () => <div data-testid="scoreboard">Scoreboard</div> }));
jest.mock('./TeamLogo', () => ({ __esModule: true, default: (p: any) => <span data-testid={`logo-${p.team || p.alt}`}>{p.team || p.alt}</span> }));
jest.mock('./StatComparisonRow', () => ({ __esModule: true, default: ({ stat }: any) => <div data-testid={`stat-${stat}`}>{stat}</div> }));
jest.mock('./FloatingVideoPlayer', () => ({ __esModule: true, default: ({ isVisible, label }: any) => isVisible ? <div data-testid="video-player">Video {label}</div> : null }));
...
const buildGameData = () => ({
  ...
  rightRail: {
    ...
    teamSeasonStats: {
      contextLabel: 'REG', contextSeason: '20242025',
      awayTeam: { ppPctg: 10, ppPctgRank: 20, pkPctg: 80, pkPctgRank: 5, faceoffWinningPctg: 52, faceoffWinningPctgRank: 11, goalsForPerGamePlayed: 3.1, goalsForPerGamePlayedRank: 7, goalsAgainstPerGamePlayed: 2.7, goalsAgainstPerGamePlayedRank: 9 },
      homeTeam: { ppPctg: 15, ppPctgRank: 12, pkPctg: 82, pkPctgRank: 3, faceoffWinningPctg: 51, faceoffWinningPctgRank: 14, goalsForPerGamePlayed: 2.9, goalsForPerGamePlayedRank: 12, goalsAgainstPerGamePlayed: 2.5, goalsAgainstPerGamePlayedRank: 6 }
    },
    last10Record: {
      awayTeam: { record: '6-3-1', streakType: 'W', streak: 2, pastGameResults: [ { opponentAbbrev: 'ABC', gameResult: 'W' }, { opponentAbbrev: 'DEF', gameResult: 'L' } ] },
      homeTeam: { record: '5-4-1', streakType: 'L', streak: 1, pastGameResults: [ { opponentAbbrev: 'XYZ', gameResult: 'L' }, { opponentAbbrev: 'QRS', gameResult: 'W' } ] }
    },
    seasonSeries: [ {
      id: 1001,
      gameType: 2,
